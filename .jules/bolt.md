...
## 2026-03-09 - Optimize isChapterRead Lookup in Render Loop
**Learning:** Calling context helper functions that internally execute O(N) operations (e.g., `Array.some()` inside `isChapterRead`) within a render loop like `.map()` causes a severe O(N*M) performance bottleneck, as arrays grow.
**Action:** Replace the usage of O(N) context helpers within loops with a locally memoized structure like a `Set` (e.g., `useMemo(() => new Set(...))`) to achieve O(1) lookups and prevent O(N*M) complexity.
