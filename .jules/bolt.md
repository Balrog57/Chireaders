...
## 2024-05-10 - Optimize Massive Datasets in React Native
**Learning:** In-memory search filtering of massive datasets in React Native blocks the JS thread, and FlatList without strict virtualization props causes significant memory and layout calculation overhead.
**Action:** Always debounce local search filtering (e.g., 300ms `setTimeout`) to prevent JS thread blocking, and implement `FlatList` virtualization props (`initialNumToRender`, `maxToRenderPerBatch`, `windowSize`, `removeClippedSubviews`) to control memory and layout calculation times.
