...
## 2025-05-27 - Optimizing Massive Lists in React Native
**Learning:** Rendering massive in-memory datasets and performing local search filtering on every keystroke blocks the JS thread and degrades scrolling performance and typing responsiveness.
**Action:** Always debounce local search filtering (e.g., with a 300ms `setTimeout` in `useEffect`) and implement `FlatList` virtualization props (`initialNumToRender`, `maxToRenderPerBatch`, `windowSize`, `removeClippedSubviews`) to control memory and layout calculation times.
