...
## 2025-05-27 - Memoize Large Computed Datasets in Context
**Learning:** Optimization Pattern: Context providers supplying large computed datasets (like history data in StorageContext) must memoize the resulting data using `useMemo` instead of wrapping the computation in a `useCallback` function, preventing severe performance degradation from re-computing on every consumer render.
**Action:** When a computed dataset needs to be exposed via a context, compute and memoize it inside the provider with `useMemo`, and expose the value directly instead of exposing a function to generate it.
