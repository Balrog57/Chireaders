...
## 2024-05-04 - Debounce search and virtualize list for massive datasets
**Learning:** Filtering a massive in-memory dataset synchronously on every keystroke in React Native blocks the JS thread and causes UI stuttering. Similarly, rendering large lists without virtualization causes high memory usage and layout calculation times.
**Action:** Always debounce local search filtering (e.g., 300ms setTimeout in useEffect) and implement FlatList virtualization props (initialNumToRender, maxToRenderPerBatch, windowSize, removeClippedSubviews) for large datasets like the library catalog.
