...
## 2026-03-09 - Avoid O(N) context lookups in render loops
**Learning:** Calling context helper functions that internally execute O(N) operations (e.g., `Array.some()`) inside render loops like `FlatList` `renderItem` or `.map()` causes O(N*M) performance bottlenecks and global UI stuttering on state updates.
**Action:** Pre-calculate a `Set` or `Map` using `useMemo` outside the loop to achieve O(1) lookups. Ensure raw data dependencies are explicitly destructured from the `useContext` hook to avoid undefined variable errors.
