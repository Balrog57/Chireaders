...
## 2024-05-24 - Missing Virtualization on Long Lists
**Learning:** In React Native, `FlatList` components without virtualization properties (like `initialNumToRender`, `windowSize`, `removeClippedSubviews`) can cause significant memory overhead and slow initial rendering for unbounded lists.
**Action:** Always include virtualization properties on `FlatList` for dynamic or potentially long lists to minimize initial render time and background memory usage.
