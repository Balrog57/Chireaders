...
## 2026-04-18 - Avoid O(N) context helpers in render loops
**Learning:** Using context helper functions that internally execute O(N) operations (e.g., `isChapterRead` calling `Array.some()`) inside render loops like `FlatList` `renderItem` or `.map()` causes O(N*M) performance bottlenecks and global UI stuttering on state updates as the list grows.
**Action:** Always pre-calculate a `Set` or `Map` using `useMemo` outside the render loop based on the raw data dependencies (e.g., `readChapters`) to achieve O(1) lookups during rendering.
