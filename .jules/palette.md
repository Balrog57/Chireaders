...
## 2025-05-27 - [Disabled Button Accessibility State]
**Learning:** Using the `disabled` prop on React Native interactive components (like `TouchableOpacity`) prevents interaction but does not reliably announce the disabled state to screen readers. It must be explicitly paired with `accessibilityState={{ disabled: true }}` to guarantee proper accessibility tree generation.
**Action:** Always include `accessibilityState={{ disabled: boolean }}` when implementing disabled or loading states on touchable components.
