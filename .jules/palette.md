...
## 2024-05-22 - Touch Targets
**Learning:** Icon-only buttons (like those in app headers) often have physical sizes under the recommended 44x44px touch target size, making them difficult to tap accurately.
**Action:** Always apply `hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}` to `TouchableOpacity` components containing only small icons to increase the interactive touch target size without altering the visual layout.
