...
## 2026-05-11 - Target Size for Icon Buttons
**Learning:** Small icon-only buttons (like those in HomeScreen header using style `styles.iconButton`) have physical dimensions under the recommended 44x44px, making them difficult to tap, particularly for users with motor impairments.
**Action:** Always apply `hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}` to these `TouchableOpacity` components to increase their interactive touch target size without altering the visual layout.
