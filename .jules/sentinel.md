...
- **Solution** : Vérifier l'URL d'origine du message dans le handler `onMessage`.
  ...
  ```
## 2025-05-02 - Insecure Deserialization in BackupService
**Vulnerability:** Backup files and library cache retrieved via StorageAccessFramework are parsed with JSON.parse and returned blindly without schema validation.
**Learning:** External files can be modified or corrupted, and parsing them without validating their type and shape can lead to application crashes or prototype pollution-like issues when expected data structures (like arrays) are missing.
**Prevention:** Always validate the structure of parsed JSON (e.g., Array.isArray, typeof object, property checks) before returning it from storage services.
