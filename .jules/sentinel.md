...
## 2025-05-11 - Dormant DOM XSS in WebView Injections
**Vulnerability:** Use of `innerHTML` for static string assignments in WebView injected JavaScript.
**Learning:** Even if the assigned string is static, using `innerHTML` creates a dormant DOM XSS vulnerability that could be exploited if the code is later refactored to use dynamic data.
