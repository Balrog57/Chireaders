...
## 2024-05-24 - DOM XSS Prevention in WebView Injections
**Vulnerability:** Use of `innerHTML` for static text assignment (`indicator.innerHTML = ' ✓'`) inside WebView injected JavaScript environments.
**Learning:** While assigning a static string is not immediately exploitable, using `innerHTML` creates a dormant DOM-based XSS vulnerability. If the code is later refactored to include dynamic or user-controlled input, it can be trivially exploited.
**Prevention:** Always use `textContent` over `innerHTML` when assigning text strings in WebView injected JavaScript to ensure safe DOM manipulation and prevent potential XSS vulnerabilities.
