...
const BrowserScreen = ({ route }) => {
    ...
    const handleMessage = (event) => {
        // Validation stricte de l'origine pour prévenir les injections depuis des domaines non fiables
        if (!isValidChiReadsUrl(event.nativeEvent.url)) {
            console.warn(`[Security] Ignored message from untrusted origin: ${event.nativeEvent.url}`);
            return;
        }
