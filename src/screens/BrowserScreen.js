...
const BrowserScreen = ({ route }) => {
    ...
    const handleMessage = (event) => {
        // Validation de sécurité : s'assurer que le message provient bien de chireads.com
        if (!isValidChiReadsUrl(event.nativeEvent.url)) {
            console.warn(`[Securité] Message ignoré car l'URL d'origine n'est pas valide: ${event.nativeEvent.url}`);
            return;
        }
