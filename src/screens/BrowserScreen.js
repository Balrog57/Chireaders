...
const BrowserScreen = ({ route }) => {
    ...
    const handleMessage = (event) => {
        try {
            // Security: Validate message origin to prevent processing
            // potentially malicious messages from untrusted domains
            if (!isValidChiReadsUrl(event.nativeEvent.url)) {
                console.warn('Blocked message from untrusted origin:', event.nativeEvent.url);
                return;
            }
