...
const NovelDetailScreen = () => {
    ...
    // ⚡ Bolt: Pre-calculate a Set of read chapter URLs outside the render loop
    // to achieve O(1) lookups and prevent O(N*M) performance bottlenecks
    // when mapping through potentially hundreds of chapters.
    const readChapterUrls = useMemo(() => {
        return new Set((readChapters[url] || []).map(ch => ch.url));
    }, [readChapters, url]);
