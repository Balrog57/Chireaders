...
const NovelDetailScreen = () => {
    ...
    // ⚡ Bolt: Pre-calculate read chapters into a Set to avoid O(N*M) lookups
    // in the render loop via `isChapterRead()`, improving render performance for novels with many chapters.
    const readChaptersSet = useMemo(() => {
        const seriesRead = readChapters[url] || [];
        return new Set(seriesRead.map(ch => ch.url));
    }, [readChapters, url]);
