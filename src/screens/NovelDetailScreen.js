...
const NovelDetailScreen = () => {
    ...
    // ⚡ Bolt: Pre-calculate a Set of read chapters for O(1) lookups
    // This prevents O(N*M) complexity when rendering large chapter lists
    // where isChapterRead (Array.some) would otherwise be called in a loop.
    const readChaptersSet = useMemo(() => {
        const seriesChapters = readChapters[url] || [];
        return new Set(seriesChapters.map(ch => ch.url));
    }, [readChapters, url]);
