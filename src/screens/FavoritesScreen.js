...
const FavoritesScreen = ({ navigation }) => {
    ...
    const renderFavoriteItem = useCallback(({ item }) => {
        ...
        return (
            <TouchableOpacity
                ...
                accessibilityLabel={`${item.title}, ${progress.length} chapitre${progress.length > 1 ? 's' : ''} lu${progress.length > 1 ? 's' : ''}, ${lastChapter ? 'dernier lu: ' + lastChapter.title : ''}`}
    ...
    }, [theme, handleFavoritePress, handleDeletePress, toggleFavoriteNotification, getSeriesProgress]);
