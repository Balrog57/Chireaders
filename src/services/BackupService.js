...
import { isSAFFileMatch } from '../utils/SAFUtils.mjs';
...
const BackupService = {
    ...
    async autoBackup(data) {
        try {
            ...
            const backupFile = files.find(uri => isSAFFileMatch(uri, BACKUP_FILE_NAME));
    ...
    async restoreFromBackup(folderUri = null) {
        try {
            ...
            for (const fileUri of files) {
                if (isSAFFileMatch(fileUri, BACKUP_FILE_NAME)) {
    ...
    async saveFile(filename, content) {
        try {
            ...
            const files = await StorageAccessFramework.readDirectoryAsync(folderUri);
            const existingFile = files.find(uri => isSAFFileMatch(uri, filename));
    ...
    async readFile(filename) {
        try {
            ...
            const files = await StorageAccessFramework.readDirectoryAsync(folderUri);
            const fileUri = files.find(uri => isSAFFileMatch(uri, filename));
