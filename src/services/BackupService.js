            ...
            for (const fileUri of files) {
                if (isMatchingFile(fileUri, BACKUP_FILE_NAME)) {
                    ...
                    try {
                        ...
                        if (parsed && typeof parsed === 'object' && (parsed.favorites || parsed.readChapters || parsed.settings)) {
                            return parsed;
                        }
                        console.error("[Security] Invalid backup schema detected");
                        return null;
                    } catch (e) {
                        console.error("[Security] Failed to parse backup file securely", e);
                        return null;
                    }
                }
            }
    ...
    async loadLibraryCache() {
        ...
        if (content) {
            try {
                ...
                console.error("[Security] Invalid library cache schema: expected an array");
                return null;
            } catch (e) {
                console.error("[Security] Failed to parse library cache securely", e);
                return null;
            }
        }
        return null;
